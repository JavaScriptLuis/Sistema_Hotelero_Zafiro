    ...
    <v-container fluid>
        <v-card class="elevation-3 rounded-lg">
            ...
            <v-card-text>
                ...
                <v-data-table :headers="headers" :items="pisos" :search="search" :items-per-page="10"
                    class="elevation-2 rounded-lg custom-table" :no-data-text="'No hay datos disponibles'"
                    :no-results-text="'No se encontraron resultados'" :items-per-page-text="'Piso por página'"
                        ...
                        <v-row>
                            <v-col cols="12" sm="6">
                                <v-text-field :error-messages="errors.nombre" v-model="form.nombre"
                                    label="Número de piso" variant="outlined" class="rounded-lg"
...
import usePiso from '@/services/usePiso';
...
const { getPisos, pisos, storePiso, updatePiso, desactivePiso, activePiso } = usePiso();
const openDialog = (modo, element = null) => {
    if (modo === 'crear') {
        dialogTittle.value = 'Registrar piso';
        dialog.value = true;
        accion.value = 'crear';
    } else if (modo === 'editar') {
        dialogTittle.value = 'Editar piso';
        dialog.value = true;
        accion.value = 'editar';
        if (element) {
            form.id = element.id
